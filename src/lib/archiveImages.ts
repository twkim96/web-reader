import type { BlobWriter, Entry, FileEntry, ZipReader } from '@zip.js/zip.js';
...
export const selectArchiveImageEntries = (entries: Entry[]): ArchiveInspection => {
  if (entries.length > MAX_ARCHIVE_ENTRIES) {
    throw new ArchiveImageError('too-many-entries', '압축 파일의 항목 수가 너무 많습니다.');
  }

  const imageEntries = entries.flatMap((entry): ArchiveImageEntry[] => {
    if (entry.directory) return [];
    const normalizedName = normalizeArchivePath(entry.filename);
...
const openArchive = async (blob: Blob) => {
  ...
  try {
    ...
    return { reader, inspection: selectArchiveImageEntries(entries) };
  } catch (error) {
...
const makeArchiveBook = (
  reader: ZipReader<Blob>,
  entries: ArchiveImageEntry[],
  fileName: string,
  BlobWriterClass: typeof BlobWriter,
): FoliateBook => {
  ...
  const loadPage = async (index: number): Promise<string> => {
    ...
    const loadPromise = (async () => {
      ...
      try {
        const imageBlob = await entry.source.getData(new BlobWriterClass(entry.mimeType));
        if (destroyed) throw new Error('Archive image source is closed.');
...
export const createZipImageBook = async (blob: Blob, fileName: string) => {
  ...
  return makeArchiveBook(reader, inspection.entries, fileName, BlobWriter);
};
