...
interface CloudModeSelectorProps {
  theme: AuthTheme;
  userName: string;
  isPublicPC: boolean;
  onPublicPCChange: (isPublicPC: boolean) => void;
  onLogout: () => void;
  onConnect: () => void;
  onLocalMode: () => void;
}

export const CloudModeSelector: React.FC<CloudModeSelectorProps> = ({
  theme,
  userName,
  isPublicPC,
  onPublicPCChange,
  ...
  <div className="h-screen w-screen flex flex-col items-center justify-center gap-8 p-10 text-center">
    ...
    <div className="w-full max-w-xs space-y-4">
      ...
      <label className={`flex items-center justify-center gap-3 p-4 rounded-2xl border-2 transition-all cursor-pointer ${isPublicPC ? 'border-accent-500 bg-accent-500/10' : `border-transparent ${theme.secondary}`}`}>
        <input type="checkbox" checked={isPublicPC} onChange={(e) => onPublicPCChange(e.target.checked)} className="hidden" />
