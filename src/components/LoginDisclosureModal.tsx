  ...
  return (
    ...
    >
      <section
        ...
        data-login-disclosure-mode={mode}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4">
          ...
          <div data-login-disclosure-notice="true" className={`mt-4 border-t pt-4 ${theme.border}`}>
            ...
            <section className={mode === 'firebase' ? 'mt-4' : 'mt-3'}>
              <div className="mb-1.5 flex items-center gap-1.5 opacity-70">
                <HardDrive size={14} aria-hidden="true" />
                <h4 className="text-[11px] font-black">Google Drive 연결{mode === 'firebase' ? ' (선택)' : ''}</h4>
              </div>
              <ul className="space-y-1 text-[10px] font-semibold leading-[1.55] opacity-55">
                {mode === 'firebase' && <li>• 로그인 후 클라우드 연결을 선택할 때 별도의 Drive 동의를 요청합니다.</li>}
