...
- 상단 제목 캡슐은 메뉴와 같은 텍스트 크기로 키우고 박스 padding을 늘린 뒤, 메뉴 크기 조정에 맞춰 약 10% 줄였다. 실제 제목 폭과 `X` 버튼 위치를 측정해, 화면 전체 중앙정렬이 안전한 제목은 중앙에 두고 `X`와의 10px 안전 여백을 위협하는 긴 제목만 오른쪽 여백을 예약한 뒤 왼쪽 공간까지 더 사용하도록 분기했다. `X` 버튼도 제목 박스와 균형이 맞는 크기로 조정했다.
- 제목, `X`, 우하단 메뉴 표면은 반투명 테마 배경과 inline `backdropFilter`/`WebkitBackdropFilter`를 적용해 본문 위에서 살짝 블러된 floating control처럼 보이도록 맞췄다.
