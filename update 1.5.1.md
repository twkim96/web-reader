...
- 우하단 메뉴는 폭을 줄이고 각 row 높이, 텍스트 크기, 아이콘 크기를 키워 모바일/iPad에서 가시성을 높였다.
- 진행률 이동 확인 모달은 중앙 배치를 유지하고, 설정/테마/목차 같은 패널형 모달만 위쪽 배치를 사용한다.
- 상단 제목 캡슐은 메뉴와 같은 텍스트 크기로 키우고 박스 padding을 늘렸다. 실제 제목 폭과 `X` 버튼 위치를 측정해, 화면 전체 중앙정렬이 안전한 제목은 중앙에 두고 `X` 안전 여백을 위협하는 긴 제목만 오른쪽 여백을 예약한 뒤 왼쪽 공간까지 더 사용하도록 분기했다. `X` 버튼도 제목 박스 크기에 맞춰 더 키웠다.
