...
- 2026-07-06 기준 1.6.5 구현과 자동 검증을 완료했다.
- 실사용 테스트와 배포 검증은 아직 진행하지 않았다.

## 목표

- PDF와 ZIP/CBZ/7z 이미지 리더에서 작은 이미지를 일시적으로 확대해 볼 수 있게 한다.
- 확대/축소는 별도 버튼 UI 없이 모바일 pinch, PC `Ctrl`+휠, PC `Ctrl`+위/아래 방향키로 처리한다.
...
- PC에서는 `Ctrl`+마우스 휠로 확대/축소하고, 이 입력은 브라우저 기본 페이지 zoom과 리더 페이지 넘김으로 새지 않게 막는다.
...
- 구현 완료.
- fixed-layout 상대 확대 API, React 오버레이 pinch/`Ctrl` 입력 처리, 페이지 이동 초기화를 반영했다.
- 로컬 자동 검증과 production 브라우저 회귀를 통과했다.

### 완료 조건

- PDF 페이지에서 pinch, `Ctrl`+휠, `Ctrl`+위/아래 방향키로 확대/축소할 수 있다.
...
- 확대 입력은 브라우저 자체 페이지 zoom이나 의도치 않은 다음/이전 페이지 이동을 발생시키지 않는다.
- PDF 고배율 렌더에서도 기존 canvas 크기 보호 한도가 유지된다.

### 검증

- `tests/browserRegression.mjs`에 fixed-layout 확대 API와 페이지 이동 초기화 회귀를 추가했다.
- `npm run test:formats`: 36개 통과.
- `npm run test:archives`: 32개 통과.
- `npm run test:browser`: 통과.
- 실사용 모바일 pinch 감각은 실제 기기 테스트 항목으로 남긴다.
...
- 남은 항목은 실제 모바일/트랙패드/마우스 환경에서 pinch와 `Ctrl`+휠 감각을 확인하는 실사용 테스트다.
