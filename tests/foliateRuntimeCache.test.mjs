...
test('uses one release version for the Foliate entry and current runtime cache', () => {
  assert.equal(FOLIATE_RUNTIME_VERSION, '1.8.1-hotfix.2');
  assert.equal(FOLIATE_RUNTIME_CACHE_NAME, 'pc-reader-v1.8.1-hotfix.2');
  assert.equal(FOLIATE_ENTRY_URL, '/foliate-js/view.js?v=1.8.1-hotfix.2');
});
...
test('removes only Foliate entries from stale release caches', async () => {
  ...
  const cacheStorage = createCacheStorage({
    ...
    'pc-reader-v1.8.1-hotfix.2': [
      `${origin}/foliate-js/view.js?v=1.8.1-hotfix.2`,
    ],
    'unrelated-cache': [
      `${origin}/foliate-js/view.js`,
    ],
  ...
  assert.deepEqual([...cacheStorage.stores.get('pc-reader-v1.8.1-hotfix.2')], [
    `${origin}/foliate-js/view.js?v=1.8.1-hotfix.2`,
  ]);
