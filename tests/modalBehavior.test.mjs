...
test('mobile menu sheets use a floating height-bounded themed surface', async () => {
  ...
  assert.match(globals, /data-viewer-menu-style='glass'\]\s*\{[^}]*--app-menu-control-filter:\s*none/);
  ...
  assert.match(globals, /\.app-modal-close\s*\{[\s\S]*?background-color:\s*var\(--app-menu-close-surface, var\(--app-menu-control-surface\)\)[\s\S]*?backdrop-filter:\s*var\(--app-menu-control-filter, var\(--app-menu-filter\)\)/);
});
...
test('keeps provisional navigation controls available while ordinary panels hide reader chrome', async () => {
  ...
  assert.match(source, /const shouldHideReaderChrome = isBaseReaderPanelOpen[\s\S]*?\|\| translation !== null \|\| syncConflict !== null \|\| Boolean\(tts\.state\.mode\)/);
  assert.match(source, /showControls=\{\(chrome\.showControls \|\| pendingSliderMove !== null \|\| isSliderPreviewing\) && !shouldHideReaderChrome\}/);
...
test('keeps theme headers and action-footers outside the scrolling modal body', async () => {
  ...
  assert.match(globals, /data-viewer-menu-style='glass'\]\s*\{[^}]*--app-menu-control-shadow:[^}]*0 1px 2px/);
  assert.match(globals, /data-viewer-menu-style='glass'\]\s*\{[^}]*--app-menu-control-filter:\s*none/);
  assert.match(globals, /data-viewer-menu-style='glass'\] \.app-menu-sheet-action\s*\{[\s\S]*?box-shadow:\s*var\(--app-menu-control-shadow\)[\s\S]*?backdrop-filter:\s*var\(--app-menu-control-filter\)/);
});
...
test('uses compact square theme and menu-style previews with custom-only accent controls', async () => {
  ...
  assert.match(themeSource, /data-menu-style-texture-preview="true"[\s\S]*?text-\[9px\][\s\S]*?sm:text-\[10px\][^>]*>\{label\}<\/span>/);
