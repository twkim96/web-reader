...
test('builds a numbered chapter label from a normalized 32-grapheme excerpt', () => {
  const content = `\n\n  첫 문장은   여러 공백과\n줄바꿈을 포함하지만 목차에서는 한 줄로 보입니다. ${'가'.repeat(40)}`;
  const label = buildTxtChapterLabel(content, 2);

  assert.match(label, /^3\. 첫 문장은 여러 공백과 줄바꿈을 포함하지만 목차에서는/);
...
test('writes numbered excerpts into new TXT EPUB navigation and escapes XML', async () => {
  ...
  assert.match(nav, /<a href="ch001\.xhtml">1\. 첫 장의 시작 &amp; &lt;비밀&gt;은 목차에도 안전하게/);
  assert.match(nav, /<a href="ch002\.xhtml">2\. 둘째 장은 전혀 다른 구절로 시작해서 위치를 기억하기/);
  assert.match(firstChapter, /<title>제목 &amp; &lt;테스트&gt; - 1\. 첫 장의 시작 &amp; &lt;비밀&gt;/);
});
