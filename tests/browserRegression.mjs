...
try {
  ...
  const serviceWorkerResult = await evaluate(`(async () => {
    ...
    const expectedCache = 'pc-reader-v1.8.29';
    ...
    const registration = await navigator.serviceWorker.register(
      '/sw.js?browser-regression=1.8.29',
      { scope: '/' },
    );
  ...
  assert.deepEqual(serviceWorkerResult.cacheNames, ['pc-reader-v1.8.29']);
  ...
  assert.match(serviceWorkerResult.scriptUrl, /\/sw\.js\?browser-regression=1\.8\.29$/);
