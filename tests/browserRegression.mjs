...
try {
  ...
  const serviceWorkerResult = await evaluate(`(async () => {
    ...
    const expectedCache = 'pc-reader-v1.7.5.1';
    ...
    const registration = await navigator.serviceWorker.register(
      '/sw.js?browser-regression=1.7.5.1',
      { scope: '/' },
    );
  ...
  assert.deepEqual(serviceWorkerResult.cacheNames, ['pc-reader-v1.7.5.1']);
  assert.equal(serviceWorkerResult.oldCacheDeleted, true);
  assert.ok(serviceWorkerResult.preCacheHits.every(({ cached }) => cached));
  assert.match(serviceWorkerResult.scriptUrl, /\/sw\.js\?browser-regression=1\.7\.5\.1$/);
