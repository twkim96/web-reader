...
| 1.8.32 | 메뉴 글래스·빈 책장 온보딩 개편 | 표준/글래스/모던 3종 한 줄 배치·각 카드 surface 미리보기·Blue→Midnight·테마색 240px 빈 책장 액션·표지/metadata 포함 장문 로컬 샘플 동화·책장 기준 `#태그` 권수·스크롤 빈 surface 메뉴 호출·기본 본문 20px | 낮음 | **완료** · 전체 check·production browser regression·PC/390px 시각검증 통과, Android/iPad 실기기 확인 대기 |
