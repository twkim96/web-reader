...
전체 상태: 1.8.10 장기 실사용 검증을 유지하면서 1.8.11 편의성 개발을 시작했다. 1.8.11의 첫 범위는 도서 정보 모달과 비정기 Firebase 플랫폼 메타데이터 게시다.
...
| 1.8.11 | 도서 정보·플랫폼 메타데이터 | 길게 누르기 정보창·안전 삭제·읽기 전용 메타데이터 조회 | 중간 | Phase A·B 구현, 자동검증·게시·실기기 확인 진행 중 |
...
상태: 길게 누르기 도서 정보창과 2단계 삭제, `file_check` 최신 플랫폼 카탈로그의 Firebase 읽기 전용 bucket projection을 구현하고 전체 자동 gate, `web-novel-viewer` Rules 배포와 최초 256개 bucket 게시를 완료했다. 외부 리뷰·실기기 확인 대기 — `update_1.8.11.md`
