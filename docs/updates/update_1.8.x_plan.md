...
| 1.8.31 | 책장 표지 레이아웃 개선 | 리스트 84px 행 안정화·가변 제목+우측 형식/progress compact 4열·초록 로컬 태그를 장르 앞에 통합·리스트 총 10-chip 상한·그리드 대형 표지+4줄 제목+TXT/조회수 표지 하단 정렬·전체폭 태그 최대 2줄·compact 카드 높이·progress baseline/삭제 아이콘 정렬·도서 정보 제목 옆 로컬 썸네일 | 낮음 | 구현·BookCard layout·실브라우저 좌표 회귀·release/cache 정합성 검증 |
