...
전체 상태: 1.8.3 개발·배포 완료. 미커밋 hotfix.1·hotfix.2 코드·전체 자동검증 완료, 외부 리뷰와 누적 실기기 검증 대기
...
| 1.8.3-hotfix.1 | 동기화 데이터 안전성 보강 | 로컬 ID·삭제 generation·aggregate 크기·누적 read | 높음 | 구현·full gate 완료, 외부 재리뷰 대기 |
| 1.8.3-hotfix.2 | 충돌 해결 일관성 보강 | canonical local·일회성 progress command·UI gating·generation barrier | 높음 | 코드·전체 자동검증 완료, 외부 재리뷰 대기 |
...
`1.8.2 — 메모·팔레트·주석 관리 UI`까지 코드 구현, 반복 리뷰, 전체 자동 gate와 push를 마쳤다. 실기기 검증 완료로 판정한 것은 아니며, 사용자 결정에 따라 1.8.3 동기화 구현 뒤 누적 기능을 한 흐름으로 검증한다.

현재 다음 단계는 다음 순서로 진행한다.

1. `docs/updates/update_1.8.3.md`를 만들고 annotation·palette 동기화의 schema, outbox, revision, tombstone, hydration 경계를 현재 checkout에서 확정한다.
2. 1.8.3을 작은 동기화 단위로 구현하고 Rules·transaction·listener·owner generation 회귀와 전체 자동 gate를 통과시킨다.
3. PC↔Android/iPad에서 선택·하이라이트·메모·팔레트·목록과 양방향 동기화를 통합 실기기 검증한다.
4. 브라우저 탭·PWA, offline·재연결, background·foreground, 동시 수정·삭제와 PWA update 전후 복원을 최소 2~3일 확인한다.
5. 실기기 결함은 1.8.4 기능과 섞지 않고 영향 범위와 안정화 patch 여부를 먼저 판단한다.
