...
전체 상태: 1.8.5 외부 리뷰 finding 수정·full gate 완료, 코드 릴리스 마감. 1.8.6 구현 시작. 1.8.0~1.8.9 코드·리뷰 종료 뒤 1.8.10 누적 실사용 안정화 예정
...
1. `docs/updates/update_1.8.6.md` 기준으로 선택·현재 위치 기본 TTS를 구현한다.
2. 1.8.6 전체 자동 gate와 외부 코드 리뷰의 타당한 finding을 정리한 뒤 별도 커밋한다.
3. 같은 방식으로 1.8.7~1.8.9를 기능별 코드·자동검증·외부 리뷰 단위로 진행한다.
