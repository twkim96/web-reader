...
- 리더 툴바의 듣기 버튼은 단일 문장 대신 현재 위치부터 현재 장 연속 듣기를 바로 시작한다.
- 장 끝 기본 동작은 `다음 장 계속 듣기`로 변경한다.
- 기존 저장 설정은 한 번만 새 연속 듣기 기본값으로 이전하며, 이후 사용자가 `현재 장에서 멈춤`을 선택하면 그 선택을 보존한다.

### shelf 태그 반응형 표시

- 모바일 리스트는 장르·태그·`+n`을 실제 카드 가로폭에 맞춰 계산하여 한 줄로만 표시한다.
- 데스크톱 리스트는 기존 최대 5개 표시를 유지하고, 그리드 카드는 확보된 공간을 활용해 작품의 모든 태그를 표시한다.
- 라이브러리 주석 모달은 내부 조작을 유지하면서 바깥 배경 클릭·터치로 닫을 수 있게 한다.

### 기본 테마와 현재 책장 태그 집계

- 새 설정의 기본 테마는 `dark`, 기본 포인트 컬러는 `yellow`로 통일한다.
- 필터의 인기 태그 순서와 작품 수는 전체 공개 카탈로그가 아니라 현재 열어 둔 로컬 또는 클라우드 책장의 도서만 집계한다.
- 현재 책장에 존재하는 태그는 작품 수가 많은 순으로 숫자와 함께 먼저 표시하고, 0개인 태그는 숫자 없이 기존 전체 인기순으로 뒤에 이어 표시한다.
- 집계는 이미 로드된 `PreparedShelfBook`의 정수 태그 ID를 모달에서 한 번 순회하며 Firebase 추가 조회를 만들지 않는다.
