...
- 진행률 드래그 프리뷰는 최대 폭/높이를 제한하고 긴 챕터명은 2줄까지만 표시해 메뉴가 흔들리지 않게 했다.
...
- 우하단 메뉴는 폭을 줄이고 각 row 높이, 텍스트 크기, 아이콘 크기를 키워 모바일/iPad에서 가시성을 높인 뒤, 제목/X와 균형이 맞도록 전체 크기와 간격을 약 10% 줄였다.
- 우하단 메뉴는 넓은 PC 화면에서 viewport 오른쪽이 아니라 가운데 정렬된 리더 텍스트 영역(`maxInlineSize: 1000px`)의 오른쪽 경계를 기준으로 배치한다.
- 진행률 이동 확인 모달은 중앙 배치를 유지하고, 설정/테마/목차 같은 패널형 모달만 위쪽 배치를 사용한다.
- 상단 제목 캡슐은 메뉴와 같은 텍스트 크기로 키우고 박스 padding을 늘린 뒤, 메뉴 크기 조정에 맞춰 약 10% 줄였다. 실제 제목 폭과 `X` 버튼 위치를 측정해, 화면 전체 중앙정렬이 안전한 제목은 중앙에 두고 긴 제목은 `X`와의 6px 안전 여백을 예약한 뒤 왼쪽 공간까지 더 사용하도록 분기했다. `X` 버튼도 제목 박스와 균형이 맞는 크기로 조정했다.
- 제목, `X`, 우하단 메뉴 표면은 반투명 테마 배경과 inline `backdropFilter`/`WebkitBackdropFilter`를 적용해 본문 위에서 살짝 블러된 floating control처럼 보이도록 맞췄다. blur가 묻히지 않도록 표면 투명도를 낮추고, WebKit에서 단계적으로 진해져 보이는 문제를 줄이기 위해 메뉴 컨테이너의 opacity fade 대신 transform만 애니메이션한다.
- 상단 제목/`X` 컨테이너의 표시 애니메이션 속도를 우하단 메뉴와 같은 `duration-200 ease-out`으로 맞췄다.
