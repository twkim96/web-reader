...
상태: TXT 목차 개선 및 hotfix.1~7 통계 수렴·Android 연속 기록·1분 확정·즉시 조회·검색창 및 라이브러리 주석 밀도 개선 구현, 전체 자동 gate 완료. 누적 실사용 검증 진행
...
- [hotfix.7](./update_1.8.10-hotfix.7.md)에서 라이브러리 전체 주석 창을 통계 모달과 같은 폭·높이 정책으로 줄이고 목록의 가로 넘침을 차단했다.
