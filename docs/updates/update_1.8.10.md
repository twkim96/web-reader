...
상태: TXT 목차 개선 및 hotfix.1~12 누적 안정화 구현. 누적 실사용 검증 진행
...
- [hotfix.11](./update_1.8.10-hotfix.11.md)에서 통계 화면의 완료 동작과 완료 상태를 `완료하기`·`완료됨`으로 구분하고, 주요 버튼·수치를 사용자가 고른 포인트 색상으로 통일했다.
- [hotfix.12](./update_1.8.10-hotfix.12.md)에서 통계 모달의 포인트 색상을 현재 설정값에 직접 결합하고, 보조 버튼 테두리는 테마 중립색으로 복원해 과도한 색상 강조를 줄였다.
