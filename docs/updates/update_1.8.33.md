...
- 심플 모드의 진행 정보 날짜를 다른 보기와 같은 `연·월·일·시·분` 형식으로 확장한다. 표지 아래 메타데이터에는 플랫폼별 수치를 합산한 조회수를 확장자 뒤에 추가한다. 제목만 두 줄로 고정하고 메타데이터와 진행 정보는 각각 한 줄 안에서 말줄임 처리한다.
- PC 책장 작업 바는 스크롤 위치와 무관하게 항상 화면 하단에 고정하고, 로그인·로그아웃만 라이브러리 헤더 오른쪽에 둔다. 모바일은 필터·보기 전환·로그인 상태 버튼을 같은 상단 행에 배치한다.
- 라이브러리 헤더 하단과 첫 도서 행 사이의 합산 여백을 PC 44px에서 16px로 줄이고, 모바일도 별도 제어 행을 제거해 책장이 헤더에 더 가깝게 시작하도록 한다.
- 자동 생성 임시 표지는 제공된 두 Adobe Color 팔레트의 10색을 교차 순서로 사용한다. 책 ID별 색은 재실행해도 고정하되 현재 테마 배경과 명도 대비가 1.5:1 미만인 후보는 건너뛰어 표지가 책장 배경에 묻히지 않게 한다.
- 책장 헤더는 PC·모바일 모두 안전 영역 아래 여백을 8px로 줄인다. 라이브러리명은 모바일 21px·PC 22px, 상태 아이콘은 31px, 필터·보기·로그인 상태 아이콘은 22px로 약 10% 확대하되 기존 40px 터치 영역은 유지한다.
- 샘플 도서 추가는 기존 이솝 우화 본문과 첫 도서 ID 호환성을 유지하면서, 달빛·새벽·햇살·장미·안개·라일락·노을·숲의 서로 다른 SVG 표지 8권을 로컬 책장에 한 번에 설치한다.
- 상단 헤더를 위로 당긴 후에도 책 목록이 너무 붙어 보이지 않도록, 책장 콘텐츠 시작 여백을 8px에서 12px로 미세 조정한다.
- 책장 하단 메뉴바는 PC에서도 모바일과 같은 34px 곡률을 사용해 화면 폭이 바뀌어도 외곽 인상이 급격히 네모나게 바뀌지 않게 한다.
- 글래스 책장 메뉴바는 테마 글자색 대신 배경 명도에서 파생한 전용 흑·백 아이콘 색을 사용한다. 4px 저블러는 유지하고 뒤 표지의 채도·대비만 압축하며, 비활성 아이콘에 반대 명도의 1px 외곽을 더해 투명감과 가독성을 함께 유지한다. 표준·모던 메뉴는 변경하지 않는다.
- 리더 본문 검색창의 위치·높이·최대 크기·진입 표현을 Shelf 검색창 기준으로 통일한다.
