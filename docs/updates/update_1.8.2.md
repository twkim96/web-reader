...
상태: 개발·리뷰·전체 자동 gate 완료 — 코드 릴리스 마감, 통합 실기기 검증은 1.8.3 이후로 이관
...
상태: 완료 — 코드 리뷰 finding 수정·전체 재검증 완료, 실기기 검증은 1.8.3 동기화 이후 통합 수행
...
- 사용자 결정에 따라 1.8.3 동기화 이후 통합 실기기 검증으로 이관
...
- 실제 Android/iPad/PWA 검증 결과는 1.8.3 동기화 이후 통합 실기기 검증에서 자동화 결과와 별도로 기록한다.
...
- 손가락·펜·마우스의 짧고 정지된 `pointerup`이 렌더링된 하이라이트 범위에 있으면 하이라이트 메뉴를 즉시 열고, synthetic click처럼 `pointerup`이 없는 기존 Foliate `show-annotation` 경로도 fallback으로 유지한다.
...
## 2차 Web GPT 리뷰 P3 수용 결과

통합 기록 화면의 검색 결과 노출과 툴바 접근성·발견성 finding 두 건을 UI 범위에서 수정했다.

- 검색어 또는 `메모 있음` 필터가 활성화되면 결과가 있는 색상 그룹만 임시로 펼쳐 결과 행을 즉시 표시한다.
- 검색·필터를 해제하면 사용자가 검색 전에 유지하던 그룹 접힘 상태로 돌아간다.
- 통합 진입점의 접근성 이름을 `책갈피와 주석`으로 바꾸고, 책갈피·주석 개수를 별도 접근성 설명과 툴팁으로 제공한다.
- 기존 책갈피 숫자 표시는 유지하고 주석이 있으면 작은 점을 표시해 좁은 모바일 툴바에서 주석 존재를 알린다.
- annotation 저장·undo·owner 격리·동기화 계약은 변경하지 않는다.

## 자동검증 결과

2026-08-02 2차 리뷰 수정과 실기기 UI 피드백 반영 후 현재 checkout에서 다음을 통과했다.
...
- production Chrome regression: 통과
  ...
  - 기본 접힘 상태에서 검색·메모 필터 결과 그룹 자동 펼침과 필터 해제 후 접힘 상태 복원
  - 통합 진입점의 책갈피·주석 접근성 이름, 개수 설명과 주석 존재 표시
...
- 2026-08-03 사용자 결정으로 1.8.2 코드 릴리스를 마감하고, 실기기 검증은 annotation·palette 동기화가 포함되는 1.8.3 이후 통합 수행한다.
- Android Chrome 브라우저·PWA와 iPad Safari 탭·홈 화면 PWA에서 선택·하이라이트·메모·목록·동기화를 한 흐름으로 검증한다.
- 색상당 20개·총 100개 실제 목록의 스크롤, 소프트 키보드, 화면 회전과 PC↔모바일 양방향 동기화를 함께 확인한다.
- 이번 마감은 자동 gate와 리뷰가 끝난 코드 범위의 완료이며, 실기기 검증 완료를 의미하지 않는다.
