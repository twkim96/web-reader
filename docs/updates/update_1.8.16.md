...
- 표면은 현재 테마 명암에 따라 Muzio의 light `white/88` 또는 dark `#1f1f1f/88`을 사용한다.
...
- Web Reader의 dark/light/sepia/blue/custom theme 배경 명암을 판정해 Muzio light/dark surface를 선택한다.
...
- Chromium 기반 자동 시각 검증에서 Muzio dark surface `rgba(31,31,31,0.88)`, mobile radius 16px, 24px blur, 2xl shadow와 icon opacity 84%가 적용된 것을 확인했다. 실제 iPad Safari와 Android Chrome의 렌더링은 실기기 gate로 남긴다.
